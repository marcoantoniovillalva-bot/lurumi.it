...
// ---------- Pagina ----------
export default function TutorialEditImagePage() {
    ...
    const [subjectY, setSubjectY] = useState(0.5);
        ...
        const load = async () => {
            try {
                ...
                if (user) {
                    ...
                    const storagePath = `${user.id}/tutorials/${id}/${imgId}`;
    ...
    const handleSave = async (mode: 'replace' | 'new') => {
        ...
        try {
            ...
            if (mode === 'replace') {
                // Sostituisce l'immagine originale nel tutorial
                await luDB.saveFile({ id: imgId, blob });
                const storagePath = `${user.id}/tutorials/${id}/${imgId}`;
                await supabase.storage.from('project-files').upload(storagePath, blob, { upsert: true });
                // imgId già presente in tutorial.images — nessuna modifica alla lista
            } else {
                // Aggiunge come nuova immagine
                const newImgId = `${tutorial.id}_tut_img_${Date.now()}`;
                await luDB.saveFile({ id: newImgId, blob });
                const storagePath = `${user.id}/tutorials/${id}/${newImgId}`;
                await supabase.storage.from('project-files').upload(storagePath, blob, { upsert: true });
                const updatedImages = [...(tutorial.images || []), { id: newImgId }];
    ...
    if (!tutorial) {
        return <div className="p-10 text-center font-bold">Tutorial non trovato</div>;
    }
        ...
        <div className="flex flex-col h-screen bg-[#0E0E10] text-white overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-[#18181B] border-b border-white/10 flex-shrink-0">
                ...
                <div className="text-center">
                    ...
                    <p className="text-sm font-black truncate max-w-[180px]">{tutorial.title}</p>
                </div>
                <button
